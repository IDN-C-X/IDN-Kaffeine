namespace :scheduler do
  ...
  task ping_all: :environment do
    ...
    elsif (7.hours.ago.strftime('%H:%M') <= site.bedtime and Time.now.strftime('%H:%M') >= site.bedtime)
      Rails.logger.info "it's during the site's bedtime! not pinging #{c}/#{total} (#{site.id}) #{Time.now} #{url} #{site.bedtime}"
    elsif (site.bedtime[0..1].to_i >= 17 and (7.hours.ago.strftime('%H:%M') <= site.bedtime) and Time.now.strftime('%H:%M') <= ("0" + (site.bedtime[0..1].to_i - 18).to_s + site.bedtime[2..4]))
      Rails.logger.info "it's during the site's bedtime which is after 17:00! not pinging #{c}/#{total} (#{site.id}) #{Time.now} #{url} #{site.bedtime}"
    else
